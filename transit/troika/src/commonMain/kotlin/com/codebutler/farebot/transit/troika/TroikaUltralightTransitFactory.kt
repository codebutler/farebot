...
private fun convertDateTime1992(
    days: Int,
    mins: Int,
): Instant? =
    if (days == 0 && mins == 0) {
        ...
        TROIKA_EPOCH_1992 + (days - 1).days + mins.minutes
    }
...
private fun convertDate2019(days: Int): Instant? =
    if (days == 0) {
        ...
        TROIKA_EPOCH_2019 + (days - 1).days
    }

private fun convertDateTime2019(
    days: Int,
    mins: Int,
): Instant? =
    if (days == 0 && mins == 0) {
        ...
        TROIKA_EPOCH_2019 + (days - 1).days + mins.minutes
    }

private fun convertDateTime2016(
    days: Int,
    mins: Int,
): Instant? =
    if (days == 0 && mins == 0) {
        ...
        TROIKA_EPOCH_2016 + (days - 1).days + mins.minutes
    }
...
abstract class TroikaBlock(
    ...
    companion object {
        ...
        fun getSerial(rawData: ByteArray) = rawData.getBitsFromBuffer(20, 32).toLong() and 0xffffffffL

        private fun getTicketType(rawData: ByteArray) = rawData.getBitsFromBuffer(4, 16)
        ...
        fun parseBlock(rawData: ByteArray): TroikaBlock =
            when (getLayout(rawData)) {
                0x2 -> TroikaLayout2(rawData)
                0xa -> TroikaLayoutA(rawData)
                0xd -> TroikaLayoutD(rawData)
                0xe ->
                    when (rawData.getBitsFromBuffer(56, 5)) {
...
private class TroikaUnknownBlock(
    rawData: ByteArray,
) : TroikaBlock(rawData) {
    override val info: List<ListItemInterface>
        get() =
