...
abstract class Trip {
    ...
    companion object {
        ...
        suspend fun formatStationNames(trip: Trip): String? {
            val startStationName = trip.startStation?.displayName?.resolveAsync()

            val endStationName: String?
            if (trip.endStation?.stationName == trip.startStation?.stationName &&
                trip.endStation?.shortStationName == trip.startStation?.shortStationName
            ) {
                endStationName = null
            } else {
                endStationName = trip.endStation?.displayName?.resolveAsync()
            }
