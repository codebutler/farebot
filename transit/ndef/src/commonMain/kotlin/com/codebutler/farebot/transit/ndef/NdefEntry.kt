...
import farebot.transit.ndef.generated.resources.*
...
sealed class NdefEntry {
    ...
    private val headInfo: List<ListItemInterface>
        get() =
            listOfNotNull(
                HeaderListItem(FormattedString(name)),
                id?.let {
                    ListItem(
                        Res.string.ndef_id,
    ...
    override fun toString(): String =
        "[name=$name, id=$id, tnf=$tnf, type=${type.toHexDump()}, payload=${payload.toHexDump()}]"
}
...
data class NdefText(
    ...
    val text: String
        get() =
            if (isUTF16) {
                payload.readUTF16BOM(
                    isLittleEndianDefault = false,
                    start = langLen + 1,
                )
            } else {
                payload.readUTF8(langLen + 1)
            }

    override fun toString(): String = "[Text: id=$id, language=$language, isUTF16=$isUTF16, value=$text]"
}
...
data class NdefWifi(
                    ...
                    when (it.type) {
                        ...
                        0x1049 ->
                            if (it.value.size >= 5 &&
                                ...
                                ListItemRecursive(
                                    FormattedString(Res.string.ndef_wifi_wfa_extension),
                                    null,
                        ...
                        0x1061 ->
                            ...
                            ListItem(
                                FormattedString(Res.string.ndef_wifi_unknown, it.type.toString(16)),
                                FormattedString(it.value.toHexDump()),
                            )
                    }
                }.toList()
    ...
    companion object {
        private fun infoWfaExtension(payload: ByteArray): List<ListItemInterface> =
            entriesFromBytes(payload.sliceOffLen(3, payload.size - 3), 1)
                .map {
                    when (it.type) {
                        0 ->
                            ListItem(
        ...
        private fun formatBitmap(
            ...
            for (i in bitmapDefinition.indices) {
                ...
                builder.append(FormattedString(bitmapDefinition[i]))
            }
            ...
            for (i in bitmapDefinition.size until (8 * len)) {
                ...
                builder.append(FormattedString(Res.string.ndef_wifi_bitmap_unknown, i))
            }
