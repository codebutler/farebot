...
class TouchnGoTransitFactory : TransitFactory<ClassicCard, TouchnGoTransitInfo> {
    ...
    override fun parseInfo(card: ClassicCard): TouchnGoTransitInfo {
        ...
        val block1_0 = sector1.getBlock(0).data
        val isTravelPass = block1_0.byteArrayToInt(0, 2) == 0x3233
                && block1_0.byteArrayToInt(4, 2) == 0x5230
                && block1_0.byteArrayToInt(10, 2) == 0x4602
        ...
        val issueCounter = block1_0.byteArrayToInt(12, 2)
        val issueDate = parseDaystamp(block1_0, 14)
    ...
    companion object {
        ...
        private fun parseTollTrip(sector: DataClassicSector): TouchnGoGenericTrip? {
            return TouchnGoGenericTrip.parse(sector, Trip.Mode.TOLL_ROAD)
        }

        private fun parsePosTrip(sector: DataClassicSector): TouchnGoGenericTrip? {
            return TouchnGoGenericTrip.parse(sector, Trip.Mode.POS)
        }

        private fun parseTransitTrip(sector: DataClassicSector): TouchnGoTrip? {
            return TouchnGoTrip.parse(sector)
        }

        private fun parseInProgressTrip(sector: DataClassicSector): TouchnGoInProgressTrip? {
            return TouchnGoInProgressTrip.parse(sector)
        }

        private fun parseRefill(sector: DataClassicSector): TouchnGoRefill? {
            return TouchnGoRefill.parse(sector)
        }
    }
}
