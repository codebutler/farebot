...
object Crypto1Recovery {
     ...
     */
    fun lfsrRecovery32(ks2: UInt, input: UInt): List<Crypto1State> {
        ...
        val transformedInput = ((input shr 16) and 0xFFu) or
            (input shl 16) or
            (input and 0xFF00u)
        ...
        recover(
            oddArr, oddArr.size, oks,
            evenArr, evenArr.size, eks,
            11, results, transformedInput shl 1,
        )
     ...
     */
    private fun extendTableSimpleInPlace(tbl: UIntArray, endIdx: Int, bit: Int): Int {
        var end = endIdx
        var idx = 0
     ...
     */
    private fun updateContribution(data: UIntArray, idx: Int, m1: UInt, m2: UInt) {
        val item = data[idx]
        var p = item shr 25
    ...
    private fun recover(
        ...
        for ((bucket, oddIndices) in oddBuckets) {
            ...
            recover(
                oddSub, oddSub.size, oksLocal,
                evenSub, evenSub.size, eksLocal,
                remLocal, results, inputLocal,
            )
        }
    }
     ...
     */
    fun nonceDistance(n1: UInt, n2: UInt): UInt {
        var state = n1
