...
object Crypto1Auth {
     ...
     */
    fun computeReaderResponse(state: Crypto1State, nR: UInt, nT: UInt): Pair<UInt, UInt> {
     ...
     */
    fun verifyCardResponse(state: Crypto1State, aTEnc: UInt, nT: UInt): Boolean {
     ...
     */
    fun encryptBytes(state: Crypto1State, data: ByteArray): ByteArray {
     ...
     */
    fun decryptBytes(state: Crypto1State, data: ByteArray): ByteArray {
        return ByteArray(data.size) { i ->
            (data[i].toInt() xor state.lfsrByte(0, false)).toByte()
        }
    }
