...
package com.codebutler.farebot.card.classic.pn533

import com.codebutler.farebot.card.classic.crypto1.Crypto1Auth
import com.codebutler.farebot.card.classic.crypto1.Crypto1State
...
class PN533RawClassic(
     ...
     */
    suspend fun requestAuth(keyType: Byte, blockIndex: Int): UInt? {
        disableCrc()
        disableParity()
        clearCrypto1()
     ...
     */
    suspend fun authenticate(keyType: Byte, blockIndex: Int, key: Long): Crypto1State? {
     ...
     */
    suspend fun nestedAuth(keyType: Byte, blockIndex: Int, currentState: Crypto1State): UInt? {
     ...
     */
    suspend fun readBlockEncrypted(blockIndex: Int, state: Crypto1State): ByteArray? {
    ...
    companion object {
         ...
         */
        fun buildAuthCommand(keyType: Byte, blockIndex: Int): ByteArray {
         ...
         */
        fun parseNonce(response: ByteArray): UInt {
            return bytesToUInt(response)
        }
        ...
        fun bytesToUInt(bytes: ByteArray): UInt {
            return ((bytes[0].toInt() and 0xFF).toUInt() shl 24) or
