...
class HomeViewModel(
    ...
    private val _errorMessage = MutableStateFlow<Pair<String, String>?>(null)
    val errorMessage: StateFlow<Pair<String, String>?> = _errorMessage.asStateFlow()
    ...
    fun startObserving() {
        ...
        viewModelScope.launch {
            cardScanner.scanErrors.collect { error ->
                val (title, message) = categorizeError(error)
                ...
                _errorMessage.value = title to message
            }
        }
    }
    ...
    fun emitSampleCard() {
        viewModelScope.launch {
            processScannedCard(RawSampleCard())
        }
    }
    ...
    private suspend fun categorizeError(error: Throwable): Pair<String, String> {
        val className = error::class.simpleName ?: ""
        return when {
            className == "CardUnauthorizedException" ->
                getString(Res.string.locked_card) to getString(Res.string.keys_required)
            error.message?.contains("Tag was lost", ignoreCase = true) == true ->
                getString(Res.string.tag_lost) to getString(Res.string.tag_lost_message)
            else ->
                getString(Res.string.error) to (error.message ?: getString(Res.string.unknown_error))
        }
    }
    ...
    private suspend fun processScannedCard(rawCard: RawCard<*>) {
        try {
            ...
            _errorMessage.value = getString(Res.string.error) to (e.message ?: getString(Res.string.failed_to_process_card))
        }
    }
}
