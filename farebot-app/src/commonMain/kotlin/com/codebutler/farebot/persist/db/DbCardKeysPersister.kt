...
class DbCardKeysPersister(private val db: FareBotDb) : CardKeysPersister {
    ...
    override fun getSavedKeys(): List<SavedKey> =
        db.savedKeyQueries.selectAll().executeAsList().map { it.toSavedKey() }

    override fun getForTagId(tagId: String): SavedKey? =
        db.savedKeyQueries.selectByCardId(tagId).executeAsOneOrNull()?.toSavedKey()
    ...
    override fun insert(savedKey: SavedKey): Long {
        ...
        return db.savedKeyQueries.selectAll().executeAsList().firstOrNull()?.id ?: -1
    }
...
private fun Keys.toSavedKey() = SavedKey(
    id = id,
    cardId = card_id,
