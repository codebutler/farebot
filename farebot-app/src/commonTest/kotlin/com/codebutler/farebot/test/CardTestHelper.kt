...
object CardTestHelper {
    ...
    fun standardFileSettings(fileSize: Int): StandardDesfireFileSettings {
        return StandardDesfireFileSettings.create(
            fileType = 0x00,
            commSetting = 0x00,
    ...
    fun recordFileSettings(recordSize: Int, maxRecords: Int, curRecords: Int): RecordDesfireFileSettings {
    ...
    fun standardFile(fileId: Int, data: ByteArray): StandardDesfireFile {
        return StandardDesfireFile(fileId, standardFileSettings(data.size), data)
    }

    fun recordFile(fileId: Int, recordSize: Int, records: List<ByteArray>): RecordDesfireFile {
    ...
    fun desfireApp(appId: Int, files: List<DesfireFile>): DesfireApplication {
    ...
    fun desfireCard(
        ...
        return DesfireCard.create(tagId, scannedAt, applications, createDesfireManufacturingData())
    }
    ...
    fun felicaSystem(code: Int, services: List<FelicaService>): FelicaSystem {
        return FelicaSystem.create(code, services)
    }

    fun felicaService(serviceCode: Int, blocks: List<FelicaBlock>): FelicaService {
        return FelicaService.create(serviceCode, blocks)
    }

    fun felicaBlock(address: Int, data: ByteArray): FelicaBlock {
        return FelicaBlock.create(address.toByte(), data)
    }

    // --- Classic Card builders ---

    fun classicBlock(type: String, index: Int, data: ByteArray): ClassicBlock {
    ...
    fun classicSector(
        ...
        return DataClassicSector(index, blocks, keyA, keyB)
    }
    ...
    fun classicCardFromSectorData(
        ...
        val sectors = (0 until numSectors).map { sectorIndex ->
            val blockData = sectorData[sectorIndex]
            if (blockData != null) {
                val blocks = blockData.mapIndexed { blockIndex, data ->
                    val type = when {
    ...
    fun hexToBytes(hex: String): ByteArray {
        return hex.replace(" ", "").hexToByteArray()
    }
}
