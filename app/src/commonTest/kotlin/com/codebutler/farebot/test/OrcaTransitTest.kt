...
class OrcaTransitTest {
    ...
    fun testDemoCard() =
        runTest {
            ...
            assertEquals("Community Transit", trips[0].agencyName?.resolveAsync())
            assertEquals("CT", trips[0].shortAgencyName?.resolveAsync())
            ...
            assertContains(trips[1].agencyName?.resolveAsync() ?: "", "Unknown")
            ...
            assertEquals("Sound Transit", trips[2].agencyName?.resolveAsync())
            assertEquals("ST", trips[2].shortAgencyName?.resolveAsync())
            ...
            val trip2StationName = trips[2].startStation?.displayName?.resolveAsync()
            if (trip2StationName == "Stadium") {
                ...
                val trip2RouteName = trips[2].routeName?.resolveAsync()
                assertTrue(
            ...
            assertEquals("Sound Transit", trips[3].agencyName?.resolveAsync())
            assertEquals("ST", trips[3].shortAgencyName?.resolveAsync())
            ...
            // Station name and route name depend on MDST being available
            val trip3StationName = trips[3].startStation?.displayName?.resolveAsync()
            if (trip3StationName == "King Street" || trip3StationName == "King St") {
                ...
                // Route name comes from MDST line name or falls back to string resource
                val trip3RouteName = trips[3].routeName?.resolveAsync()
                assertTrue(
            ...
            assertEquals("Washington State Ferries", trips[4].agencyName?.resolveAsync())
            assertEquals("WSF", trips[4].shortAgencyName?.resolveAsync())
            ...
            val trip4StationName = trips[4].startStation?.displayName?.resolveAsync()
