...
class SampleTransitDataDumpTest {
    ...
    private suspend fun processSample(sample: SampleCard): String {
        val bytes =
        ...
        val cardName = identity?.name?.resolveAsync() ?: sample.name
        ...
        return buildString {
            ...
            if (info.emptyStateMessage != null) {
                appendLine("    emptyState: \"${yamlEscape(info.emptyStateMessage!!.resolveAsync())}\"")
            }

            // Warning
            if (info.warning != null) {
                appendLine("    warning: \"${yamlEscape(info.warning!!.resolveAsync())}\"")
            }
        }.trimEnd()
    }

    private suspend fun formatTrip(
        trip: Trip,
        sb: StringBuilder,
    ) {
        ...
        sb.appendLine("        agency: ${yamlValue(trip.shortAgencyName?.resolveAsync())}")
        sb.appendLine("        route: ${yamlValue(trip.routeName?.resolveAsync())}")
        sb.appendLine("        startStation: ${yamlValue(trip.startStation?.displayName?.resolveAsync())}")
        sb.appendLine("        endStation: ${yamlValue(trip.endStation?.displayName?.resolveAsync())}")
    ...
    private suspend fun formatSubscription(
        sub: Subscription,
        sb: StringBuilder,
    ) {
        sb.appendLine("      - name: ${yamlValue(sub.subscriptionName?.resolveAsync())}")
        sb.appendLine("        agency: ${yamlValue(sub.agencyName?.resolveAsync())}")
        sb.appendLine("        validity: ${yamlValue(sub.formatValidity()?.resolveAsync())}")
        ...
        if (remaining != null) {
            sb.appendLine("        remaining: \"${yamlEscape(remaining.resolveAsync())}\"")
        }
    }

    private suspend fun formatInfoItem(
        item: ListItemInterface,
        sb: StringBuilder,
    ) {
        if (item is HeaderListItem) {
            sb.appendLine("      - header: ${yamlValue(item.text1?.resolveAsync())}")
        } else {
            sb.appendLine("      - label: ${yamlValue(item.text1?.resolveAsync())}")
            sb.appendLine("        value: ${yamlValue(item.text2?.resolveAsync())}")
        }
    }
