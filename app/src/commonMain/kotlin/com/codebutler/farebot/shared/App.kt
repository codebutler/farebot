                            ...
                            if (text != null) {
                                ...
                                when (result) {
                                    is ImportResult.Success -> {
                                        ...
                                        scope.launch {
                                            platformActions.showToast(
                                                FormattedString(Res.string.imported_cards, result.cards.size).resolveAsync(),
                                            )
                                        }
                                        historyViewModel.loadCards()
                                    }
                                    is ImportResult.Error -> {
                                        scope.launch {
                                            platformActions.showToast(
                                                FormattedString(Res.string.import_failed, result.message).resolveAsync(),
                                            )
                                        }
                                    }
                                }
                    ...
                    mapMarkers =
                        remember(supportedCards) {
                            supportedCards
                                ...
                                .map { card ->
                                    CardsMapMarker(
                                        name = FormattedString(card.nameRes),
                                        location = FormattedString(card.locationRes),
