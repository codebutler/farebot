...
class LeapTrip internal constructor(
    ...
    companion object {
        ...
        fun parseTopup(file: ByteArray, offset: Int): LeapTrip? {
            ...
            return if (amount == 0) null else LeapTrip(
                agency, Mode.TICKET_MACHINE,
                LeapTripPoint(c, -amount, -1, null), null
            )
            // 3 bytes amount after topup: we have currently no way to represent it
        }

        private fun isNull(data: ByteArray, offset: Int, length: Int): Boolean {
            return data.sliceOffLen(offset, length).isAllZero()
        }

        fun parsePurseTrip(file: ByteArray, offset: Int): LeapTrip? {
        ...
        fun parseTrip(file: ByteArray, offset: Int): LeapTrip? {
            if (isNull(file, offset, 7)) {
                return null
            }
