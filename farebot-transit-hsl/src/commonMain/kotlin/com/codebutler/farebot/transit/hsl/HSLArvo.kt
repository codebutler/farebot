...
class HSLArvo(
    ...
    override val info: List<ListItemInterface>
        get() = super.info.orEmpty() + listOfNotNull(
    ...
    companion object {
        ...
        fun parse(raw: ByteArray, version: HSLVariant): HSLArvo? {
            if (raw.isAllZero())
                return null
            ...
            return HSLArvo(
                ...
                HSLTransaction.parseEmbed(
                    raw = raw, version = version, offset = offset,
                    walttiArvoRegion = parsed.getInt(HSLLookup.contractWalttiRegionName(CONTRACT_PREFIX))
                )
            )
        }

        fun parseUL(raw: ByteArray, version: Int, city: Int): HSLArvo? {
            if (raw.isAllZero())
                return null
            if (version == 2)
                return HSLArvo(
                    ...
                    HSLTransaction.parseEmbed(
                        raw = raw, version = HSLVariant.HSL_V2,
                        offset = 264, ultralightCity = city
                    ), ultralightCity = city
                )
            return HSLArvo(
                En1545Parser.parse(raw, FIELDS_V1_UL),
                HSLTransaction.parseEmbed(
                    raw = raw, version = HSLVariant.HSL_V1,
                    offset = 264, ultralightCity = city
                ), ultralightCity = city
            )
        }
