...
fun ByteArray.getHexString(offset: Int, length: Int): String =
    ByteUtils.getHexString(this.copyOfRange(offset, offset + length))
 ...
 */
fun ByteArray.getBitsFromBuffer(iStartBit: Int, iLength: Int): Int {
    return ByteUtils.getBitsFromBuffer(this, iStartBit, iLength)
}
 ...
 */
fun ByteArray.getBitsFromBufferLeBits(iStartBit: Int, iLength: Int): Int {
 ...
 */
fun ByteArray.getBitsFromBufferSigned(iStartBit: Int, iLength: Int): Int {
 ...
 * using two's complement.
 */
fun ByteArray.getBitsFromBufferSignedLeBits(iStartBit: Int, iLength: Int): Int {
    val unsigned = getBitsFromBufferLeBits(iStartBit, iLength)
    return unsignedToTwoComplement(unsigned, iLength - 1)
}

private fun unsignedToTwoComplement(input: Int, highestBit: Int): Int =
...
fun ByteArray.byteArrayToInt(offset: Int, length: Int): Int =
    ByteUtils.byteArrayToInt(this, offset, length)

fun ByteArray.byteArrayToInt(): Int =
    ByteUtils.byteArrayToInt(this, 0, size)

fun ByteArray.byteArrayToLong(offset: Int, length: Int): Long =
    ByteUtils.byteArrayToLong(this, offset, length)

fun ByteArray.byteArrayToLong(): Long =
    ByteUtils.byteArrayToLong(this, 0, size)

fun ByteArray.byteArrayToIntReversed(offset: Int, length: Int): Int =
    byteArrayToLongReversed(offset, length).toInt()

fun ByteArray.byteArrayToIntReversed(): Int =
    byteArrayToIntReversed(0, size)

fun ByteArray.byteArrayToLongReversed(offset: Int, length: Int): Long =
    ByteUtils.byteArrayToLong(
        ByteArray(length) { this[offset + length - 1 - it] }, 0, length
    )

fun ByteArray.byteArrayToLongReversed(): Long =
    byteArrayToLongReversed(0, size)

// --- Slicing ---

fun ByteArray.sliceOffLen(offset: Int, length: Int): ByteArray =
    copyOfRange(offset, offset + length)

fun ByteArray.sliceOffLenSafe(offset: Int, length: Int): ByteArray? {
...
fun ByteArray.isASCII(): Boolean = all {
...
fun ByteArray.readUTF8(start: Int = 0, end: Int = size): String =
    decodeToString(startIndex = start, endIndex = end)

fun ByteArray.readUTF16(isLittleEndian: Boolean, start: Int = 0, end: Int = size): String {
...
fun ByteArray.readUTF16BOM(isLittleEndianDefault: Boolean, start: Int = 0, end: Int = size): String {
    if (end < start + 2) {
        return "\uFFFD"
    }
...
fun ByteArray.convertBCDtoInteger(): Int = fold(0) { x, y ->
    (x * 100) + NumberUtils.convertBCDtoInteger(y)
}

fun ByteArray.convertBCDtoInteger(offset: Int, length: Int): Int =
    sliceOffLen(offset, length).convertBCDtoInteger()

fun ByteArray.convertBCDtoLong(): Long = fold(0L) { x, y ->
    (x * 100L) + NumberUtils.convertBCDtoInteger(y).toLong()
}

fun ByteArray.convertBCDtoLong(offset: Int, length: Int): Long =
    sliceOffLen(offset, length).convertBCDtoLong()

// --- Buffer manipulation ---

fun ByteArray.reverseBuffer(): ByteArray =
    ByteArray(size) { this[size - it - 1] }
 ...
 */
inline fun ByteArray.indexOfFirstStarting(start: Int, predicate: (Byte) -> Boolean): Int {
...
fun ByteArray.indexOf(needle: ByteArray, start: Int = 0, end: Int = size): Int {
