...
object ByteUtils {
    ...
    private fun byteArrayToInt(b: ByteArray, offset: Int): Int {
        return byteArrayToInt(b, offset, b.size)
    }

    fun byteArrayToInt(b: ByteArray, offset: Int, length: Int): Int {
        return byteArrayToLong(b, offset, length).toInt()
    }

    fun byteArrayToLong(b: ByteArray): Long {
        return byteArrayToLong(b, 0, b.size)
    }
    ...
    fun byteArraySlice(b: ByteArray, offset: Int, length: Int): ByteArray {
        return b.copyOfRange(offset, offset + length)
    }

    fun convertBCDtoInteger(data: Byte): Int {
        return (((data.toInt() and 0xF0) shr 4) * 10) + (data.toInt() and 0x0F)
    }

    fun getBitsFromInteger(buffer: Int, iStartBit: Int, iLength: Int): Int {
        return (buffer shr iStartBit) and (0xFF shr (8 - iLength))
    }
     ...
     */
    fun reverseBuffer(buffer: ByteArray, iStartByte: Int, iLength: Int): ByteArray {
     ...
     */
    fun unsignedToTwoComplement(input: Int, highestBit: Int): Int {
        var inp = input
    ...
    fun getBitsFromBuffer(buffer: ByteArray, iStartBit: Int, iLength: Int): Int {
        ...
        if (iSByte == iEByte) {
            ...
            var uRet = ((buffer[iSByte].toInt() and 0xFF and (0xFF shr iSBit)) shl (((iEByte - iSByte - 1) * 8)
                    + (iEBit + 1)))
