...
actual class BundledDatabaseDriverFactory {
    actual fun createDriver(dbName: String, schema: SqlSchema<QueryResult.Value<Unit>>): SqlDriver {
        val bundlePath = NSBundle.mainBundle.pathForResource(
            dbName.removeSuffix(".db3").removeSuffix(".db"), ofType = if (dbName.endsWith(".db3")) "db3" else "db"
        )

        if (bundlePath != null) {
            // Copy bundled DB to sqliter's default path: Library/Application Support/databases/
            val appSupportDirs = NSSearchPathForDirectoriesInDomains(
                NSApplicationSupportDirectory, NSUserDomainMask, true
            )
            val appSupportDir = appSupportDirs.firstOrNull() as? String
