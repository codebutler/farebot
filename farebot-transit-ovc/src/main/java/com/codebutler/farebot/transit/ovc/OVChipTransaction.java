    ...
    static OVChipTransaction create(int transactionSlot, byte[] data) {
        ...
        if (valid == 0) {
            ...
            if ((data[3] & (byte) 0x20) != (byte) 0x00) {
                unknownConstant = ByteUtils.getBitsFromBuffer(data, iBitOffset, 24);
                iBitOffset += 24;
            }

            if ((data[3] & (byte) 0x40) != (byte) 0x00) {
                transfer = ByteUtils.getBitsFromBuffer(data, iBitOffset, 7);
                iBitOffset += 7;
            }

            if ((data[2] & (byte) 0x01) != (byte) 0x00) {
                company = ByteUtils.getBitsFromBuffer(data, iBitOffset, 16);
                iBitOffset += 16;
            }
            ...
            if ((data[2] & (byte) 0x10) != (byte) 0x00) {
                station = ByteUtils.getBitsFromBuffer(data, iBitOffset, 16);
                iBitOffset += 16;
            }
            ...
            if ((data[1] & (byte) 0x04) != (byte) 0x00) {
                vehicleId = ByteUtils.getBitsFromBuffer(data, iBitOffset, 16);
                iBitOffset += 16;
            }

            if ((data[1] & (byte) 0x10) != (byte) 0x00) {
                productId = ByteUtils.getBitsFromBuffer(data, iBitOffset, 5);
                iBitOffset += 5;
            }

            if ((data[0] & (byte) 0x01) != (byte) 0x00) {
                unknownConstant2 = ByteUtils.getBitsFromBuffer(data, iBitOffset, 16);
                iBitOffset += 16;
            }

            if ((data[0] & (byte) 0x08) != (byte) 0x00) {
                amount = ByteUtils.getBitsFromBuffer(data, iBitOffset, 16);
                iBitOffset += 16;
            }

            if ((data[1] & (byte) 0x10) == (byte) 0x00) {
                subscriptionId = ByteUtils.getBitsFromBuffer(data, iBitOffset, 13);
            }
        }
