...
class ChcMetrocardTransitFactory : TransitFactory<ClassicCard, ChcMetrocardTransitInfo> {
    ...
    override fun check(card: ClassicCard): Boolean {
        ...
        if (!file1.copyOfRange(0, ErgTransitInfo.SIGNATURE.size)
    ...
    override fun parseIdentity(card: ClassicCard): TransitIdentity {
        val metadata = ErgTransitInfo.getMetadataRecord(card)
        val serial = metadata?.cardSerial?.let { s ->
            var result = 0
            for (b in s) {
        ...
        return TransitIdentity.create(runBlocking { getString(Res.string.chc_metrocard_card_name) }, serial)
    }

    override fun parseInfo(card: ClassicCard): ChcMetrocardTransitInfo {
        val capsule = ErgTransitInfo.parse(
            card,
