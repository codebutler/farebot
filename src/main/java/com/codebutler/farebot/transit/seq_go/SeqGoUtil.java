...
public final class SeqGoUtil {
    ...
    public static GregorianCalendar unpackDate(byte[] timestamp) {
        final int minute = Utils.getBitsFromBuffer(timestamp, 5, 11);
        final int year = Utils.getBitsFromBuffer(timestamp, 16, 7) + 2000;
        final int month = Utils.getBitsFromBuffer(timestamp, 23, 4);
        final int day = Utils.getBitsFromBuffer(timestamp, 27, 5);
