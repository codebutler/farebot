...
public class OVChipIndex implements Parcelable {
	...
	public static OVChipIndex fromXML (Element element) {
		int recentTransactionSlot;
		int recentInfoSlot;
		int recentSubscriptionSlot;
		int recentTravelhistorySlot;
		...
		recentTransactionSlot = Integer.parseInt(element.getAttribute("transactionslot"));
		recentInfoSlot = Integer.parseInt(element.getAttribute("infoslot"));
		recentSubscriptionSlot = Integer.parseInt(element.getAttribute("subscriptionslot"));
		recentTravelhistorySlot = Integer.parseInt(element.getAttribute("travelhistoryslot"));
		recentCreditSlot = Integer.parseInt(element.getAttribute("creditslot"));

        for (int i = 0; i < 12; i++) {
            subscriptionIndex[i] = Integer.parseInt(element.getAttribute("indexslot" + i));
        }
	...
	public Element toXML (Document doc) throws Exception {
		...
		index.setAttribute("transactionslot", Integer.toString(mRecentTransactionSlot));
		index.setAttribute("infoslot", Integer.toString(mRecentInfoSlot));
		index.setAttribute("subscriptionslot", Integer.toString(mRecentSubscriptionSlot));
		index.setAttribute("travelhistoryslot", Integer.toString(mRecentTravelhistorySlot));
		index.setAttribute("creditslot", Integer.toString(mRecentCreditSlot));

		for (int i = 0; i <  mSubscriptionIndex.length; i++) {
			index.setAttribute("indexslot" + Integer.toString(i), Integer.toString(mSubscriptionIndex[i]));
		}
