...
import farebot.farebot_transit_rkf.generated.resources.rkf_location_gothenburg
import farebot.farebot_transit_rkf.generated.resources.rkf_location_stockholm
import farebot.farebot_transit_rkf.generated.resources.rkf_status_action_pending
import farebot.farebot_transit_rkf.generated.resources.rkf_status_not_ok
import farebot.farebot_transit_rkf.generated.resources.rkf_status_ok
import farebot.farebot_transit_rkf.generated.resources.rkf_status_temp_disabled
import farebot.farebot_transit_rkf.generated.resources.rkf_unknown_format
import kotlinx.coroutines.runBlocking
import org.jetbrains.compose.resources.getString

// Record types
sealed class RkfRecord
data class RkfSimpleRecord(val raw: ByteArray) : RkfRecord()
data class RkfTctoRecord(val chunks: List<List<ByteArray>>) : RkfRecord()

// Serial number
data class RkfSerial(val mCompany: Int, val mCustomerNumber: Long, val mHwSerial: Long) {
...
class RkfTransitInfo internal constructor(
    ...
    override val balances: List<TransitBalance> get() = mBalances.withIndex().filter { (idx, bal) ->
    ...
    val cardStatus: String
        get() = when (mTcci.getIntOrZero(STATUS)) {
            0x01 -> runBlocking { getString(Res.string.rkf_status_ok) }
            0x21 -> runBlocking { getString(Res.string.rkf_status_action_pending) }
            0x3f -> runBlocking { getString(Res.string.rkf_status_temp_disabled) }
            0x58 -> runBlocking { getString(Res.string.rkf_status_not_ok) }
            else -> runBlocking { getString(Res.string.rkf_unknown_format, NumberUtils.intToHex(mTcci.getIntOrZero(STATUS))) }
        }
    ...
    override val info: List<ListItemInterface> get() = listOfNotNull(expiryDateInfo) + listOf(
    ...
    companion object {
        ...
        internal fun getSerial(card: ClassicCard): RkfSerial {
            ...
            val hwSerial = card.sectors[0].let { sector ->
        ...
        internal val TCCI_FIELDS = En1545Container(
        ...
        internal val TCCP_FIELDS = En1545Container(
            HEADER,
            STATUS_FIELD,
            En1545Container(
...
class RkfTransitFactory : TransitFactory<ClassicCard, RkfTransitInfo> {
    ...
    override fun check(card: ClassicCard): Boolean {
        ...
        return HashUtils.checkKeyHash(sector0.keyA, sector0.keyB, "rkf",
            // Most cards
    ...
    override fun parseInfo(card: ClassicCard): RkfTransitInfo {
        ...
        for ((idx, trip) in unfilteredTrips.withIndex()) {
            if (idx > 0 && unfilteredTrips[idx - 1].startTimestamp?.toEpochMilliseconds() == trip.startTimestamp?.toEpochMilliseconds()
                && unfilteredTrips[idx - 1].checkoutCompleted && !trip.checkoutCompleted)
                continue
            if (idx < unfilteredTrips.size - 1 && unfilteredTrips[idx + 1].startTimestamp?.toEpochMilliseconds() == trip.startTimestamp?.toEpochMilliseconds()
                && unfilteredTrips[idx + 1].checkoutCompleted && !trip.checkoutCompleted)
                continue
            trips.add(trip)
        }
        ...
        for (trip in trips)
            while (i < tripTransactions.size) {
                ...
                val transactionTimestamp = RkfTransitInfo.clearSeconds(transaction.timestamp?.toEpochMilliseconds() ?: 0)
                if (transactionTimestamp > RkfTransitInfo.clearSeconds(trip.endTimestamp?.toEpochMilliseconds() ?: 0))
                    break
                i++
                if (transactionTimestamp < RkfTransitInfo.clearSeconds(trip.startTimestamp?.toEpochMilliseconds() ?: 0)) {
        ...
        return RkfTransitInfo(mTcci = tcci,
            mTrips = TransactionTripLastPrice.merge(nonTripTransactions + remainingTransactions)
                + trips.map { it.tripLegs }.flatten(),
            mBalances = balances, mLookup = lookup, mTccps = tccps, mSerial = RkfTransitInfo.getSerial(card),
