...
data class RkfTCSTTrip(
    ...
    val endTimestamp: Instant?
        get() =
            parseDateTime(
                mParsed.getIntOrZero(START_TIME) + mParsed.getIntOrZero(DESTINATION_TIME),
                mLookup.timeZone,
            )
    ...
    val fare: TransitCurrency
        get() =
            if (inProgress) {
                ...
                mLookup.parseCurrency(mParsed.getIntOrZero(PRICE))
            }
    ...
    private val endStation: Station?
        get() =
            if (checkoutCompleted) {
                mLookup.getStation(
                    mParsed.getIntOrZero(DESTINATION_PLACE),
                    mParsed.getIntOrZero(DESTINATION_AID),
                    null,
                )
            } else {
                null
        ...
        get() {
            ...
            for ((index, transaction) in mTransactions.withIndex()) {
                ...
                if (previous == null) {
                    legs.add(
                        RkfTripLeg(
                            startTimestamp = startTimestamp ?: continue,
                            endTimestamp = transaction.timestamp,
                            startStation = startStation,
                            endStation = transaction.station,
                            fare = fare,
                            passengerCount = passengerCount,
                            mode = mode,
                            isTransfer = false,
                            mShortAgencyName = getAgencyName(true),
                            mAgencyName = getAgencyName(false),
                        ),
                    )
                    continue
                }
                ...
                if (index == 1 && isCheckin(previous)) {
                    legs.add(
                        RkfTripLeg(
                            ...
                            mode = previous.mode,
                            isTransfer = false,
                            mShortAgencyName = previous.shortAgencyName,
                            mAgencyName = previous.agencyName,
                        ),
                    )
                    continue
                }
                ...
                legs.add(
                    RkfTripLeg(
                        startTimestamp = previous.timestamp ?: continue,
                        endTimestamp = transaction.timestamp,
                        startStation = previous.station,
                        endStation = transaction.station,
                        fare = null,
                        passengerCount = passengerCount,
                        mode = previous.mode,
            ...
            if (previous != null && !isCheckin(previous)) {
                legs.add(
                    RkfTripLeg(
                        startTimestamp = previous.timestamp ?: return legs,
                        endTimestamp = if (checkoutCompleted) endTimestamp else null,
                        startStation = previous.station,
                ...
                legs.add(
                    RkfTripLeg(
                        startTimestamp = startTimestamp ?: return legs,
                        endTimestamp = if (checkoutCompleted) endTimestamp else null,
                        startStation = startStation,
                        endStation = endStation,
                        fare = fare,
    ...
    private fun isCheckOut(transaction: RkfTransaction) =
        (
            transaction.isTapOff &&
                checkoutCompleted &&
                RkfTransitInfo.clearSeconds(transaction.timestamp?.toEpochMilliseconds() ?: 0) ==
                RkfTransitInfo.clearSeconds(endTimestamp?.toEpochMilliseconds() ?: 0)
        )

    private fun isCheckin(transaction: RkfTransaction) =
        (
            transaction.isTapOn &&
                RkfTransitInfo.clearSeconds(transaction.timestamp?.toEpochMilliseconds() ?: 0) ==
                RkfTransitInfo.clearSeconds(startTimestamp?.toEpochMilliseconds() ?: 0)
        )
    ...
    companion object {
        ...
        private fun passengerSubGroup(num: Int) =
            En1545Container(
