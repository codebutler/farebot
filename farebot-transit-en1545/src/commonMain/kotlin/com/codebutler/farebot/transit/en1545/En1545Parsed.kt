...
sealed class En1545Value {
    data class IntValue(val v: Int) : En1545Value()
    data class StringValue(val v: String) : En1545Value()
}

class En1545Parsed(private val map: MutableMap<String, En1545Value> = mutableMapOf()) {
    ...
    fun getInt(name: String, path: String = ""): Int? {
        return (map[makeFullName(name, path)] as? En1545Value.IntValue)?.v
    }
    ...
    fun getString(name: String, path: String = ""): String? {
        return (map[makeFullName(name, path)] as? En1545Value.StringValue)?.v
    }

    fun getTimeStamp(name: String, tz: TimeZone): Instant? {
    ...
    fun append(data: ByteArray, off: Int, field: En1545Field): En1545Parsed {
        field.parseField(data, off, "", this) { obj, offset, len -> obj.getBitsFromBuffer(offset, len) }
        return this
    }

    fun appendLeBits(data: ByteArray, off: Int, field: En1545Field): En1545Parsed {
        field.parseField(data, off, "", this) { obj, offset, len -> obj.getBitsFromBufferLeBits(offset, len) }
        return this
    }

    fun append(data: ByteArray, field: En1545Field): En1545Parsed {
        return append(data, 0, field)
    }

    fun appendLeBits(data: ByteArray, field: En1545Field): En1545Parsed {
    ...
    companion object {
        private fun makeFullName(name: String, path: String?): String {
            return if (path.isNullOrEmpty()) name else "$path/$name"
        }
